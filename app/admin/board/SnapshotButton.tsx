...
export default function SnapshotButton({ snapshotCount, snapshotTakenAt }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        <h2 className="text-sm font-semibold text-purple-500 uppercase tracking-wider mb-1">
          Player KC Snapshot
        </h2>
        <p className="text-xs text-purple-600/70 mb-3">
        ...
        {snapshotTakenAt ? (
          ...
          <p className="text-xs text-purple-700/60 mb-3">○ No snapshot taken yet</p>
        )}
        <div className="flex gap-3 flex-wrap">
          <button
            ...
            disabled={busy}
            className="px-4 py-2 rounded-lg bg-purple-700 hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-semibold transition-colors purple-glow-sm"
          >
          ...
          <button
            ...
            disabled={busy}
            className="px-4 py-2 rounded-lg bg-red-900/50 hover:bg-red-800/60 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-semibold text-red-300 border border-red-800/40 transition-colors"
          >
      ...
      {state.status === "done" && (
        <div className="text-xs text-purple-500/70 flex flex-col gap-0.5">
          {state.saved.length > 0 && (
            <p><span className="text-green-400">Saved:</span> {state.saved.join(", ")}</p>
          )}
          {state.skipped.length > 0 && (
            <p><span className="text-purple-600">Skipped (already snapshotted):</span> {state.skipped.join(", ")}</p>
          )}
