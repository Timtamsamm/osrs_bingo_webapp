...
interface Props {
  ...
  /** Extra content shown in the collapsed row header, before the chevron (e.g. stats from another source). */
...
export default function PlayerStatsCard({ memberName, temple: providedTemple, headerRight }: Props) {
    ...
    <div className="rounded-xl border border-stone-700/60 overflow-hidden">
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {!headerRight && !onDemand && !temple && (
            <p className="text-xs text-amber-500 mt-0.5">Not tracked on TempleOSRS yet</p>
          )}
          {!headerRight && onDemand && fetchedTemple?.status === "done" && !temple && (
            <p className="text-xs text-amber-500 mt-0.5">Not tracked on TempleOSRS yet</p>
          )}
        </div>
        <span
      ...
      {expanded && templeLoading && (
        <div className="border-t border-stone-700/60 bg-stone-950/90 p-4">
          <p className="text-sm text-gray-500">Loading TempleOSRS stats…</p>
        </div>
      )}

      {expanded && templeError && (
        <div className="border-t border-stone-700/60 bg-stone-950/90 p-4 flex items-center gap-3">
          <p className="text-sm text-red-400">Failed to load stats</p>
          <button type="button" onClick={loadTempleStats} className="text-xs text-gray-500 hover:text-gray-300 transition-colors">↻ Retry</button>
        </div>
      )}
