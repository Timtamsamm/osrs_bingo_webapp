...
type Tile = { id: string; title: string; tiers: TierDef[] };
...
export default function ManualAwardForm({ teams, tiles }: { teams: Team[]; tiles: Tile[] }) {
  ...
  const availableTiers = useMemo(() => selectedTile?.tiers ?? [], [selectedTile]);

  async function submit() {
    if (!teamId || !tileId || !tier) return;
    setLoading(true);
    setMessage(null);
    const res = await fetch("/api/admin/submissions/manual", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ teamId, tileId, tier, note }),
    });
    setLoading(false);
    if (res.ok) {
    ...
    <div className="bg-[#0e0820] border border-purple-900/40 rounded-xl p-5 flex flex-col gap-3 mb-10">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="flex flex-col gap-1.5 min-w-[200px]">
          ...
          <select
            value={tileId}
            onChange={(e) => { setTileId(e.target.value); setTier(""); }}
            className={inputCls}
          >
            <option value="">Select tile…</option>
            {tiles.filter((t) => t.tiers.length > 0).map((t) => (
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={submit}
          disabled={loading || !teamId || !tileId || !tier}
